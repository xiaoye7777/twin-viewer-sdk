...
export type ViewerRuntimeState = DeepReadonly<{ projectId:string|null; bindings:TwinBinding[]; runtimeValues:Record<string,TwinRuntimeValue>; resolutionByBindingId:Record<string,'resolved'|'unresolved'>; bindingRevision:number; runtimeRevision:number; resolutionRevision:number; mockRunning:boolean; mockTickCount:number; dataSourceType:DataSourceType; dataSourceStatus:DataSourceConnectionStatus; dataSourceMessageCount:number; dataSourceError:string|null }> & { getRuntimeValue(bindingId:string,variableKey:string):DeepReadonly<TwinRuntimeValue>|null }
export interface ViewerDiagnostics { dataSource:{type:DataSourceType;status:DataSourceConnectionStatus;messageCount:number;error:string|null}; visualRules:{activations:number;activeRules:number}; effects:{effects:number;transientOwners:number;helpers:number;outlined:number} }
export interface TwinSceneViewerPublicApi { focusDevice(deviceId:string):Promise<boolean>; focusTarget(target:TwinBindingTarget):Promise<boolean>; selectDevice(deviceId:string):boolean; selectTarget(target:TwinBindingTarget):boolean; clearSelection():void; getSelection():ViewerSelection; getRuntimeState():ViewerRuntimeState|null; setDataSource(config:ViewerDataSourceConfig):boolean; getDiagnostics():ViewerDiagnostics }
...
export const TwinSceneViewer: DefineComponent<{ source:TwinPackageSource; dataSource?:ViewerDataSourceConfig }>
